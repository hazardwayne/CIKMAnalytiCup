...
np.random.seed(seed)
model.fit([
           X_train], [y_train], epochs=6, verbose=2, batch_size=24)

test_predict = model.predict([
                               X_test], batch_size=24)


np.savetxt(os.path.join(ensemble_dir, 'clarity_test_mlp_%d.predict' % seed), test_predict, fmt='%.6f')